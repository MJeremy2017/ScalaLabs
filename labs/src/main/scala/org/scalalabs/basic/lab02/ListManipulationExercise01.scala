...
object ListManipulationExercise01 {
  ...
  def lastElementInList[T](l: List[T]): T = {
    // l.last
    l match {
      case h :: tail => lastElementInList(tail)
      case h :: Nil => h
    }
  }
  ...
  def nthElementInList[T](n: Int, l: List[T]): T = {
    l(n)
    l.zipWithIndex.filter(_._2 == n).head._1
  }
  ...
  def concatLists[T](l1: List[T], l2: List[T]): List[T] = {
    ...
    // pattern match
    def myConcat[T](l1: List[T], l2: List[T]): List[T] = {
      l1 match {
        case Nil => l2
        case h :: t => h :: myConcat(t, l2)
      }
    }
  ...
  def sortList[T <% Ordered[T]](list: List[T]): List[T] = {
    list.foldLeft(List[T]())( (acc, v) => {
      val (l1, l2) = acc.span(x => x < v)
      l1 ::: v :: l2
    })
  }
